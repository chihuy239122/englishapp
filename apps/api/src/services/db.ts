...
export async function requireSession(db: D1Database, sessionId: string): Promise<{ id: string; user_id: string; persona: string; level: string }> {
  const session = await db.prepare("SELECT id, user_id, persona, level FROM sessions WHERE id = ?").bind(sessionId).first<{ id: string; user_id: string; persona: string; level: string }>();
